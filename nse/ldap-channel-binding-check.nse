...
description = [[
Checks whether LDAP channel binding (LdapEnforceChannelBinding) is enforced on
a Windows domain controller via an unauthenticated GSS-SPNEGO/NTLM SASL bind
over TLS.

A SPNEGO NegTokenInit wrapping an NTLM Type 1 (NEGOTIATE) with no signing
flags is sent over the TLS-protected channel; after receiving the Type 2
challenge wrapped in a NegTokenResp, a SPNEGO NegTokenResp wrapping a null
NTLM Type 3 (AUTHENTICATE) follows.  resultCode 49 (invalidCredentials) means
the DC accepted the unsigned negotiate -- channel binding is not required.
resultCode 8 (strongAuthRequired) means enforced.
...
---
-- @usage nmap -p 636 --script ldap-channel-binding-check <host>
-- @output
-- 636/tcp open  ldaps
...
portrule = shortport.port_or_service({636, 3269}, {"ldapssl", "ldaps"}, "tcp",
                                     {"open", "open|filtered"})
...
action = function(host, port)
  ...
  if code == RC_INVALID_CREDENTIALS then
    return "Channel Binding: NOT REQUIRED"
  end
  return nil
end
